...
function initSPARouter() {
    ...
    async function navigateWithTransition(url) {
        let wipe = document.querySelector(".page-transition-wipe");
        if (!wipe) {
        ...
        // 1. Drop curtain
        await gsap.to(wipe, { y: 0, duration: 0.4, ease: "power3.inOut" });
        
        try {
            // 2. Fetch new page instantly without full reload
            ...
            const currentPath = new URL(url, window.location.href).pathname.split('/').pop() || 'index.html';
            ...
            // 8. Re-trigger scroll animations and re-init logic for new main content
            ScrollTrigger.refresh();
            
            // Re-bind specific listeners if needed for new elements
            if(currentPath === 'index.html') {
                if(typeof initHomeLogic === 'function') initHomeLogic();
            }

        } catch (e) {
            console.error("SPA Fetch failed, falling back to hard reload", e);
            window.location.href = url;
            return;
        }

        // 9. Lift curtain smoothly
        gsap.to(wipe, { y: "-100%", duration: 0.5, ease: "power3.inOut" }).then(() => {
            gsap.set(wipe, { y: "100%" }); // Reset position for next time
            window.scrollTo(0, 0); // Reset scroll to top smoothly
        });
    }
}
...
function initQuiz() {
    ...
    // Use event delegation for the hero button so it works after SPA transition
    document.body.addEventListener("click", (e) => {
        const heroBtn = e.target.closest(".hero .cta-button");
        if (heroBtn && !e.target.closest(".quiz-container")) {
            e.preventDefault();
            overlay.classList.add("active");
            // Cinematic blur entrance
            gsap.to(overlay, { opacity: 1, duration: 0.4, ease: "power2.out" });
            gsap.fromTo("#quiz-step-1", { opacity: 0, scale: 0.8, y: 40 }, { opacity: 1, scale: 1, y: 0, duration: 0.6, ease: "back.out(1.2)", delay: 0.1 });
        }
    });
    ...
    document.querySelectorAll(".quiz-btn").forEach(btn => {
        btn.addEventListener("click", function() {
            ...
            const nextStepId = "quiz-step-" + this.dataset.next;
            const nextStep = document.getElementById(nextStepId);

            if (this.dataset.profile === "light") {
                document.getElementById("quiz-result-title").innerText = "Sunrise Blend";
                document.getElementById("quiz-result-desc").innerText = "Bright, fruity profile for energetic mornings.";
            } else if (this.dataset.profile === "dark") {
                document.getElementById("quiz-result-title").innerText = "Midnight Mocha";
                document.getElementById("quiz-result-desc").innerText = "Rich, bold, perfectly suited for steady taste.";
            }

            // Smoother exit/enter stagger
            gsap.to(currentStep, { opacity: 0, x: -40, duration: 0.3, ease: "power2.in", onComplete: () => {
                currentStep.classList.remove("active");
                if(nextStep) {
                    nextStep.classList.add("active");
                    gsap.fromTo(nextStep, { opacity: 0, x: 40 }, { opacity: 1, x: 0, duration: 0.5, ease: "power2.out" });
                }
            }});
        });
    });

    const addCartBtn = document.getElementById("quiz-add-cart");
    if (addCartBtn) {
        addCartBtn.addEventListener("click", (e) => {
            gsap.to(overlay, { opacity: 0, duration: 0.3, onComplete: () => overlay.classList.remove("active") });
            
            // Trigger actual original robust cart logic
            const fakeCartAddBtn = document.createElement('button');
            fakeCartAddBtn.className = 'add-to-cart';
            fakeCartAddBtn.dataset.id = 'quiz-result';
            fakeCartAddBtn.dataset.name = document.getElementById("quiz-result-title").innerText;
            fakeCartAddBtn.dataset.price = "18.00";
            fakeCartAddBtn.style.display = 'none';
            document.body.appendChild(fakeCartAddBtn);
            fakeCartAddBtn.click();
            setTimeout(() => fakeCartAddBtn.remove(), 100);
        });
    }
}
